...
function SDX.utils:typeOf(value, type_value, default_value)
    return imports.type(value) == type_value and value or default_value;
end
...
function SDX.utils:toColor(color_data, default_color)
    ...
    if(imports.type(color_data) == 'number') then
		return {
            prepareColor(imports.bitExtract(color_data, 16, 8)),
            prepareColor(imports.bitExtract(color_data, 8, 8)),
            prepareColor(imports.bitExtract(color_data, 0, 8)),
            prepareColor(imports.bitExtract(color_data, 24, 8))
        };
    elseif(imports.type(color_data) == 'string') then
        ...
        return color;
    elseif(imports.type(color_data) == 'table') then
        local color = {
